...
RegisterNetEvent('wx_anticheat:receiveOCRWebhook')
...
function BanPlayer(reason, punish,retryCount)
    ...
    print(punish)
    TriggerServerEvent('wx_anticheat:addLog',"AntiCheat","Banned "..GetPlayerName(PlayerId()).." for "..reason,"Success")
    ...
    exports[wx.screenshotModule]:requestScreenshotUpload(OCR.Webhook, 'files[]', function(data)
        ...
        if resp ~= nil and resp.attachments ~= nil and resp.attachments[1] ~= nil and resp.attachments[1].proxy_url ~= nil then
            ...
            TriggerServerEvent("wx_anticheat:server:clientXkzuqBwmTjN7Gab4QzuN9QYZJ1WxxU", reason, SCREENSHOT_URL, punish)
        else
            if retryCount < 5 then
                Citizen.Wait(500)
                BanPlayer(reason,punish, retryCount)
            else
                TriggerServerEvent("wx_anticheat:server:clientXkzuqBwmTjN7Gab4QzuN9QYZJ1WxxU", reason, nil, punish)
            end
        end
    end)
end

RegisterNetEvent('wx_anticheat:detectclient')
AddEventHandler('wx_anticheat:detectclient',function (reason)
    BanPlayer(reason)
end)
        ...
        if wx.antiBlips then
            ...
            for _, player in ipairs(GetActivePlayers()) do
                ...
                if DoesBlipExist(GetBlipFromEntity(ped)) then
                    if not isDetected or wx.Debug then
                        isDetected = true
                        BanPlayer(Locale.Blips,wx.DetectionPunishments["blips"])
                    end
                    return
                end
            end
        ...
        while true do
            ...
            if IsPedInAnyVehicle(PlayerPedId(), false) and not IsEntityVisible(PlayerPedId()) --[[ Vehicle shop bypass ]] then
                ...
                if currentPlate ~= GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)) and GetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), false)) ~= nil then
                    if not isDetected or wx.Debug then
                        isDetected = true
                        BanPlayer(Locale.PlateChange:format(currentPlate,
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), false) and IsPedRunning(PlayerPedId()) and not IsPedRagdoll(PlayerPedId()) and not IsPedDeadOrDying(PlayerPedId(), false) and playerSpeed > 10.0 and not IsPedClimbing(PlayerPedId()) and not IsPedDiving(PlayerPedId()) and not IsPedFalling(PlayerPedId()) and not IsPedInCover(PlayerPedId()) then
                if not isDetected or wx.Debug then
                    isDetected = true
                    BanPlayer(Locale.FastRun,wx.DetectionPunishments["fastrun"])
                end
            end
        end
    end)
...
if wx.antiTeleport then
    Citizen.CreateThread(function()
        while true do
            ...
            local oldpos = GetEntityCoords(ped)
            Citizen.Wait(2500)
            if GetDistanceBetweenCoords(oldpos, GetEntityCoords(ped)) > wx.teleportDistance then
                if not IsPedInAnyVehicle(ped, false) or not IsPlayerSwitchInProgress() then
                    if not isDetected or wx.Debug then
                        isDetected = true
                        BanPlayer(("Teleport - [%s meters]"):format(math.floor(GetDistanceBetweenCoords(oldpos,
                            GetEntityCoords(ped)) + 0.5)),wx.DetectionPunishments["teleport"])
                    end
                end
            end
        end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if GetDistanceBetweenCoords(x, y, z, newx, newy, newz) > 10.0 and not IsPedInParachuteFreeFall(PlayerPedId()) and not still and not IsPedFalling(PlayerPedId()) and GetEntityHeightAboveGround(PlayerPedId()) > 5.0 and spawned and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedStill(PlayerPedId()) and not IsPedInAnyBoat(PlayerPedId()) and speed < 1.0 and not IsPedSwimming(PlayerPedId()) and not IsPedSwimmingUnderWater(PlayerPedId()) and not IsEntityInWater(PlayerPedId()) then
                if not isDetected or wx.Debug then
                    isDetected = true
                    BanPlayer(Locale.NoClip,wx.DetectionPunishments["noclip"])
                end
            end
        end
    end)
...
if wx.antiNoRecoil then
    Citizen.CreateThread(function()
        while true do
            Wait(300)
            if IsPedArmed(PlayerPedId(), 4) then
                local weapon = GetSelectedPedWeapon(PlayerPedId())
                if GetWeaponRecoilShakeAmplitude(weapon) == 0.0 then
                    BanPlayer("No Recoil")
                end
            end
        end
...
if wx.antiSuperJump then
    Citizen.CreateThread(function()
        while true do
            ...
            if wx.antiSuperJump then
                Wait(1)
                if IsPedJumping(PlayerPedId()) == 1 and GetEntityHeightAboveGround(PlayerPedId()) > 7.0 and GetEntitySpeed(PlayerPedId()) > 8.0 and not IsPedFalling(PlayerPedId()) and not IsPedDiving(PlayerPedId()) then
                    if not isDetected or wx.Debug then
                        isDetected = true
        ...
        if wx.antiThermal then
            ...
            if wx.antiFreeCam then
                ...
                if (coordX > wx.freecamDistance) or (coordY > wx.freecamDistance) or (coordZ > wx.freecamDistance) or (coordX < -wx.freecamDistance) or (coordY < -wx.freecamDistance) or (coordZ < -wx.freecamDistance) and not IsPedInAnyVehicle(PlayerPedId(), false) and not IsPedInAnyHeli(PlayerPedId()) and not IsPedInAnyTaxi(PlayerPedId()) and not IsPedInAnyPlane(PlayerPedId()) and not IsPedInAnyPoliceVehicle(PlayerPedId()) and not IsPedOnAnyBike(PlayerPedId()) and not IsPedInAnyBoat(PlayerPedId()) and spawned then
                    if not isDetected or wx.Debug then
                        isDetected = true
                        BanPlayer(Locale.FreeCam,wx.DetectionPunishments["freecam"])
                    end
                end
            end
            if wx.antiInvisible then
                if not IsEntityVisible(PlayerPedId()) and not IsEntityVisibleToScript(PlayerPedId()) or GetEntityAlpha(PlayerPedId()) <= 150 and GetEntityAlpha(PlayerPedId()) ~= 0 and spawned then
                    if not isDetected or wx.Debug then
                        isDetected = true
                        BanPlayer(Locale.Invisibility,wx.DetectionPunishments["invisibility"])
                    end
                end
            end
    ...
    AddEventHandler('onResourceStart', function(resourceName)
        if not wx.whitelistedResources[resourceName] then
            if not isDetected or wx.Debug then
                isDetected = true
                BanPlayer(
                    Locale.ResStart:format(resourceName))
            end
        end
    end)
end
    ...
    AddEventHandler('onClientResourceStop', function(resourceName)
        if not wx.whitelistedResources[resourceName] then
            if not isDetected or wx.Debug then
                isDetected = true
                BanPlayer(
                    Locale.ResStop:format(resourceName))
            end
        end
    end)
end
...
Citizen.CreateThread(function()
    while wx.antiGodMode do

        Citizen.Wait(10000)
        local curPed = PlayerPedId()
        local curHealth = GetEntityHealth(curPed)
        if curHealth-2 <=0 then
            ...
            if not IsPlayerDead(PlayerId()) then
                if PlayerPedId() == curPed and GetEntityHealth(curPed) == curHealth and GetEntityHealth(curPed) ~= 0 then
                    BanPlayer(Locale.GodMode,wx.DetectionPunishments["godmode"])
                ...
                if weapon == GetHashKey(v.weapon) then
                    if size > v.clip then
                        if not isDetected or wx.Debug then
                            isDetected = true
                            BanPlayer(
                                Locale.ClipSize:format(size, v.clip))
                        end
                    end
                end
            end
...
RegisterNUICallback('menucheck', function(data)
    if data.text ~= nil then
        for _, word in pairs(wx.OCRWords) do
...
--         TriggerServerEvent('wx_anticheat:server:CY8cV5R1F9hhguzzYbnZRYNRp4Cwn1', GetPlayerServerId(PlayerId()),
--             'cau pico')
--     end, false)
-- end
